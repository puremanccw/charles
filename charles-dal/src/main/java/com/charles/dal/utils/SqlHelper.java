...
public class SqlHelper {
	 ...
	 * @return
	 */
	private static String getMapperSql(SqlSession session, Class mapperInterface, String methodName, Object... args) {
		...
		for(int i = 0; i < argTypes.length; i++) {
			if(!RowBounds.class.isAssignableFrom(argTypes[i]) && !ResultHandler.class.isAssignableFrom(argTypes[i])) {
				String paramsName = "param" + String.valueOf(params.size() + 1);
				params.put(paramsName, i >= args.length ? null : args[i]);
			}
		}
	...
	/**
	 * 根据类型替换参数	
	 * 仅作为数字和字符串两种类型进行处理，需要特殊处理的可以继续完善
	 * @param sql
	 * @param value
	 * @param jdbcType
	 * @param javaType
